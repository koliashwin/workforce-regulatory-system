...
const HeroStat = ({ value, label, loading }) => (
  <Box sx={{ textAlign: 'center', px: 2 }}>
    {loading
      ...
      : <Typography sx={{ fontFamily: 'monospace', fontSize: '2.25rem', fontWeight: 700,
          color: '#fff', lineHeight: 1 }}>{value}</Typography>
    }
    <Typography sx={{ fontSize: '11px', color: '#718096', mt: 0.5, letterSpacing: '0.08em',
      textTransform: 'uppercase', fontFamily: 'monospace' }}>{label}</Typography>
  </Box>
);
...
const ProblemCard = ({ icon: Icon, title, desc, color }) => (
  <Box sx={{ display: 'flex', gap: 2, alignItems: 'flex-start' }}>
    <Box sx={{ width: 38, height: 38, borderRadius: '9px', background: `${color}18`,
...
const Step = ({ num, title, desc, actors }) => (
  <Box sx={{ display: 'flex', gap: 2 }}>
    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0 }}>
      <Box sx={{ width: 34, height: 34, borderRadius: '50%', background: '#0d1117', display: 'flex',
        alignItems: 'center', justifyContent: 'center', color: '#f59e0b', fontFamily: 'monospace',
        fontSize: '13px', fontWeight: 700, border: '1px solid #1e2636' }}>{num}</Box>
      {num < 5 && <Box sx={{ width: 1, flex: 1, background: '#e2e8f0', my: 0.5, minHeight: 24 }} />}
    </Box>
    <Box sx={{ pb: 3 }}>
    ...
    <Box>
      ...
      <Box sx={{
        ...
        <Container maxWidth="md" sx={{ textAlign: 'center' }}>
          <Chip label="Public Transparency Portal" size="small"
            sx={{ mb: 3, background: '#1e2636', color: '#718096', fontFamily: 'monospace',
          ...
          <Typography sx={{ fontSize: '1.0625rem', color: '#718096', lineHeight: 1.75,
          ...
          <Box sx={{ display: 'flex', gap: 2, justifyContent: 'center', flexWrap: 'wrap', mb: 6 }}>
            <Button variant="contained" size="large" endIcon={<ArrowForwardIcon />}
              ...
              sx={{ background: '#f59e0b', color: '#0d1117', fontWeight: 600, px: 3,
                '&:hover': { background: '#fbbf24' } }}>
              Browse Institutes
            </Button>
            <Button variant="outlined" size="large" endIcon={<ArrowForwardIcon />}
              onClick={() => navigate('/companies')}
              sx={{ borderColor: '#263142', color: '#a0aec0', px: 3,
        ...
        <Container maxWidth="lg">
          <Grid container spacing={6} alignItems="center">
            <Grid item xs={12} md={5}>
              ...
              <Typography sx={{ fontFamily: 'Georgia, serif', fontSize: '1.75rem', fontWeight: 700,
                  ...
                  <Card key={title} sx={{ border: '1px solid #e2e8f0' }} elevation={0}>
                    <CardContent>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 1.5 }}>
                        <Box sx={{ width: 32, height: 32, borderRadius: '8px', background: `${color}14`,
      ...
      <Box sx={{ background: '#0d1117', py: { xs: 6, md: 8 } }}>
        <Container maxWidth="sm" sx={{ textAlign: 'center' }}>
          <Typography sx={{ fontFamily: 'Georgia, serif', fontSize: '1.625rem', fontWeight: 700,
            color: '#fff', mb: 1.5 }}>
          ...
          <Button variant="contained" size="large" component="a" href="/login"
            sx={{ background: '#f59e0b', color: '#0d1117', fontWeight: 600, px: 4,
              '&:hover': { background: '#fbbf24' } }}>
            Get Started →
          </Button>
        </Container>
