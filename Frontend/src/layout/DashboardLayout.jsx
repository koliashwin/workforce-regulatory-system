...
const DashboardLayout = () => {
    ...
    return (
        <Box sx={{ display: 'flex', minHeight: '100vh', background: tokens.slate[50] }}>
            ...
            >
                {/* Logo */}
                <Box sx={{ px: 2, py: 2.5, display: 'flex', alignItems: 'center', gap: 1.25, borderBottom: `1px solid ${tokens.navy[700]}` }}>
                    ...
                    <Box>
                        <Typography sx={{ fontFamily: '"Fraunces", serif', fontWeight: 700, fontSize: '1rem', color: '#fff', lineHeight: 1.1 }}>
                            Scame
                        </Typography>
                        <Typography sx={{ fontSize: '0.65rem', color: tokens.slate[300], fontFamily: '"DM Mono", monospace', letterSpacing: '0.08em' }}>
                            REGULATORY SYSTEM
                        </Typography>
                    </Box>
                </Box>
                ...
                {/* Role badge */}
                <Box sx={{ px: 2, py: 1.5 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, background: tokens.navy[700], borderRadius: '8px', px: 1.5, py: 1 }}>
                        <RoleIcon sx={{ fontSize: 14, color: tokens.amber[400] }} />
                        <Typography sx={{ fontSize: '0.7rem', color: tokens.slate[200], fontFamily: '"DM Mono", monospace', letterSpacing: '0.06em', textTransform: 'uppercase' }}>
                        ...
                        return (
                            <ListItem key={path} disablePadding sx={{ mb: 0.25 }}>
                                <ListItemButton selected={active} onClick={() => navigate(path)}>
                                    ...
                                    <ListItemText
                                        primary={label}
                                        primaryTypographyProps={{ fontSize: '0.875rem', fontWeight: active ? 600 : 400, color: active ? '#fff' : tokens.slate[200] }}
                                    />
                                </ListItemButton>
                            </ListItem>
                        );
                ...
                <Box sx={{ px: 1, pb: 2, borderTop: `1px solid ${tokens.navy[700]}`, pt: 1.5 }}>
                    <ListItemButton onClick={() => { logout(); navigate('/login'); }} sx={{ borderRadius: '8px' }}>
                        <ListItemIcon sx={{ minWidth: 32 }}><LogoutIcon sx={{ fontSize: 17, color: tokens.slate[300] }} /></ListItemIcon>
                        <ListItemText primary="Logout" primaryTypographyProps={{ fontSize: '0.875rem', color: tokens.slate[200] }} />
                    </ListItemButton>
                </Box>
            </Drawer>
            ...
            <Box component="main" sx={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
                ...
                <AppBar position="sticky" elevation={0}>
                    <Toolbar sx={{ minHeight: '52px !important', px: 3 }}>
                        <Typography sx={{ flex: 1, fontFamily: '"DM Mono", monospace', fontSize: '0.75rem', color: tokens.slate[300], letterSpacing: '0.06em' }}>
                            EMPLOYMENT LIFECYCLE & VERIFICATION PLATFORM
                        </Typography>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                            <Box sx={{ width: 28, height: 28, borderRadius: '50%', background: tokens.navy[700], display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                <RoleIcon sx={{ fontSize: 14, color: tokens.amber[400] }} />
                            </Box>
